...
$(document).ready(function(){
    ...
    /* 브라우저가 스크롤 되면 header에 fixed 클래스 추가
        ...
        2. 다시 맨위로 올라가면 header fixed 클래스 삭제
        3. 스크롤되어있을때 새로고침 시 위의 헤더가 그대로 그자리에 있어야 함
        --> 브라우저를 스크롤 할 때도 체크해야하고
           처음에 로딩했을 때도 체크해야함
    ...
    // 문서가 로딩되었을 때 단 1번 함수 실행
    scroll_chk()
    // 브라우저가 스크롤 할 때마다 1번씩 함수 실행
    ...
    const book_swiper = new Swiper('.book .swiper', { /* 팝업을 감싼는 요소의 class명 */
        ...
        navigation: {
            nextEl: '.book .book_wrap .ctrl_btn button.btn_next',
            prevEl: '.book .book_wrap .ctrl_btn button.btn_prev',
        },
    });
