...
$(document).ready(function(){
    ...
    */
    ...
    function ceo_ani(){
        ...
        /*
            진행중일때 몇 % 스크롤을 했는지 계산해야함
            (1000px 동안 인터랙티브를 할건데 100px 스크롤 함 = 10%)
            현재스크롤값(ceo_diff) * 100 / 전체값(ceo_total)
    ...
    if(ceo_length > 0){
        ceo_ani() // 브라우저가 로딩되었을때 단 1번
    }
    $(window).scroll(function(){
        if(ceo_length > 0){
            ceo_ani() // 브라우저가 스크롤 될 때마다 1번씩
        }
    })
    ...
    /*
        ...
        4개 각각의 애니메이션 시작 시기와 종료 시기를 계산해야함
        콘텐츠가 브라우저 아래에서 위로 올라오는 위치
        스크롤 값 + 브라우저의 높이 = 컨텐츠의 offset().top 값과 동일
        ++ 브라우저 높이의 몇 10%를 더하면 애니메이션 ㅣㅅ자시기
    */
    function his_head(){
        let obj_name = $('.ctn_history .his_head') // 영역
        let obj_txt = 'h3 strong' // 애니메이션 글자
        let obj_length = obj_name.length // 저 선택자로 선택되는 요소의 개수
        ...
        for(i=0; i<obj_length; i++){
            ...
            obj_end = obj_top - win_h + (win_h * 0.8)
    ...
    $(window).resize(function(){ /* 브라우저 사이즈가 리사이즈 될때마다 실행 */
        if(history_length > 0){
            his_head()
            his_area()
            his_nav()
